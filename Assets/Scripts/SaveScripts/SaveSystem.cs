...
//     Вызвать: SaveSystem.Instance.SaveObject("уникальный_ключ", Данные);
//     Пример:
...
//     Приведите к своему типу:
//        WalletData data = raw as WalletData;
//        if (data != null) { _money = data.money; _zones = data.zones; }
//     Вызывайте в Awake() или Start() - SaveSystem уже загрузил файл в Start()
//
// 3. УНИКАЛЬНЫЕ КЛЮЧИ
...
//
// 4. ЧТО ДЕЛАТЬ, ЕСЛИ НУЖНО ИЗМЕНить ДАННЫЕ НАПРЯМУЮ (БЕЗ SAVEOBJECT)?
//     Если вы изменяете данные, которые уже сохранили, и хотите, чтобы они сохранились,
//     вызовите SaveSystem.Instance.MarkDirty()
...
//     Избегайте сохранения ссылок на Unity-объекты (GameObject, Transform) - они не сериализуются
//
// 6. АВТОСОХРАНЕНИЕ
